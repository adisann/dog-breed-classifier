...
class BreedClassifier:
    ...
    MODEL_FILE_ID = "1VbuDAwGEkJWDriGobrA_OdLrUI3FcaZl"  # Your Google Drive file ID
    ...
    def download_with_requests(self, file_id, output_path, max_retries=3):
        """Download file using requests with session for better handling of large files."""
        session = requests.Session()
        
        for attempt in range(max_retries):
            try:
                st.info(f"Downloading model using direct method (attempt {attempt+1})...")
                ...
                if 'virus scan warning' in response.text.lower() or 'download_warning' in response.text:
                    ...
                    for key, value in response.cookies.items():
                        ...
                        if token_match:
                            ...
                            raise Exception("Could not find confirmation token")
                ...
                if total_size > 0 and total_size < self.MIN_MODEL_SIZE_MB * 1024 * 1024:
                    st.warning(f"File size seems too small: {total_size} bytes")
                ...
                if os.path.exists(output_path):
                    ...
                    if file_size < self.MIN_MODEL_SIZE_MB * 1024 * 1024:
                        st.warning(f"Downloaded file is too small ({file_size} bytes). This might be an error page.")
    ...
    def download_model(self, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                st.info(f"Downloading model with gdown (attempt {attempt+1})...")
                
                # Construct the correct Google Drive URL
                ...
                if os.path.exists(self.MODEL_PATH):
                    ...
                    if file_size < self.MIN_MODEL_SIZE_MB * 1024 * 1024:  # At least 50MB
                        st.warning(f"File size smaller than expected ({file_size} bytes). This might be an access issue.")
                        os.remove(self.MODEL_PATH)
                        raise Exception("File too small, possibly corrupted or access denied.")
                    
                    st.success(f"Model downloaded successfully with gdown ({file_size / (1024*1024):.1f} MB)")
                    return True
                else:
                    raise Exception("File not found after download.")
            
            except Exception as e:
                st.warning(f"gdown failed: {str(e)}")
                if os.path.exists(self.MODEL_PATH):
                    os.remove(self.MODEL_PATH)
                break  # Try next method
        
        # Method 2: Try requests method
        st.info("Trying alternative download method...")
        if self.download_with_requests(self.MODEL_FILE_ID, self.MODEL_PATH, max_retries):
            return True
        
        # Method 3: Try with different gdown URL format
        st.info("Trying gdown with different URL format...")
        try:
            # Alternative URL format
            alt_url = f"https://drive.google.com/file/d/{self.MODEL_FILE_ID}/view?usp=sharing"
            output = gdown.download(alt_url, self.MODEL_PATH, quiet=False, fuzzy=True)
            
            if output and os.path.exists(self.MODEL_PATH):
                file_size = os.path.getsize(self.MODEL_PATH)
                if file_size >= self.MIN_MODEL_SIZE_MB * 1024 * 1024:
                    st.success(f"Model downloaded with alternative method ({file_size / (1024*1024):.1f} MB)")
                    return True
                else:
                    os.remove(self.MODEL_PATH)
        except Exception as e:
            st.warning(f"Alternative gdown method failed: {str(e)}")
        
        # All methods failed
        st.error("All download methods failed. Please check:")
        st.error("1. Make sure the Google Drive file is publicly accessible")
        st.error("2. Check if the file ID is correct")
        st.error("3. Ensure you have a stable internet connection")
        st.info(f"File ID being used: {self.MODEL_FILE_ID}")
        st.info("To make your Google Drive file public:")
        st.info("1. Right-click the file in Google Drive")
        st.info("2. Select 'Share' → 'Anyone with the link can view'")
        st.info("3. Copy the file ID from the shareable link")
            ...
            if not os.path.exists(self.MODEL_PATH):
                with st.spinner("Model not found. Downloading from Google Drive..."):
                    if not self.download_model():
                        st.error("Failed to download model. Please check the Google Drive link and try again.")
                        return False
